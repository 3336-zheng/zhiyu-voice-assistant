...
Query Rewrite（原问题 + 会话上下文 + Planner goal/intent）
  ...
  → 独立查询单次 Reranker
  → Token Budget 上下文装配
  → CRAG 基于原问题、独立查询、Planner 语义和候选证据判断
  → Evidence Gate
  → 带引用回答 / 证据不足拒答
```

Query Rewrite 在一次 LLM 调用中复用会话上下文和 Planner 语义，先完成指代消解，再生成多视角检索表达；独立查询同时作为单次 Reranker 和 CRAG 的语义基准。CRAG 还显式接收原问题及候选标题、来源、分数和有限正文，不依赖模型跨请求记忆。子块用于提高召回粒度，父块用于精排、上下文和稳定引用。多查询结果先汇总再融合和精排，避免为每个改写查询重复执行整条链路。回答中的来源保留页面、版本、章节和稳定 Chunk ID；音频页面额外支持转写时间范围回溯。
