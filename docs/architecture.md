...
| `backend/app/api/agent_feedback.py` | 回答反馈、草稿准备、确认、重试和取消协议 | 只做参数解析与错误映射 |
| `backend/app/services/agent_runtime_service.py` | Run 状态、事件、会话锁和终态持久化 | 不实现具体 Agent 工具 |
| `backend/app/services/answer_feedback_service.py` | 反馈快照、研究、确认写入、同步索引与原题复测状态机 | 复用研究、页面、PendingAction 与 Runtime，不复制执行逻辑 |
...
| `backend/app/services/memory_service.py` | 会话消息、标题、正文检索、历史恢复和增量摘要游标 | 不删除原始对话 |
| `backend/app/services/embedding_service.py` | 本地与 OpenAI 兼容 Embedding Provider、批处理与维度校验 | 不决定检索策略，不跨模型自动降级 |
| `backend/app/services/reranker_service.py` | 本地与 Cohere/Jina 兼容 Rerank Provider、响应归一化 | 不直接读取向量库，不改变候选正文 |
| `backend/app/services/chroma_service.py` | 持久化向量与 Embedding Profile 集合隔离 | 不把不同语义空间写入同一集合 |
...
| `frontend/app/src/components/AnswerFeedbackPanel.jsx` | 纠错草稿、阶段状态、前后回答与重试交互 | 不实现后端状态机 |
